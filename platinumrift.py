...
zonelink, zone1, zone2 = [], [], []
for i in range(link_count):
    zone_1, zone_2 = [int(j) for j in input().split()]
    zonelink.append([zone_1, zone_2])
    zone1.append(zone_1)
    zone2.append(zone_2)

# game loop
while True:
    ownerid, podsp0, podsp1= [], [], []
    ...
    for i in range(zone_count):
        ...
        ownerid.append(owner_id)
        podsp0.append(pods_p0)
        podsp1.append(pods_p1)
    ...
    for i in range(zone_count):
        for j in range(zone_count) :
            if mypod[i] > 0 :
                if [i,j] in zonelink :
                    move.append([mypod[i], i, j])
                elif [j,i] in zonelink:
                    move.append([mypod[i], i, j])
